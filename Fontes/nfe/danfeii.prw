...
static function gerabol(cPref,cDoc,nFaturas,aFaturas,oDanfe)
	local nDupl
	local nPosBol
...
//Adiciona os Titulos 
	...
	For nDupl := 1 To nFaturas
		If nFaturas > 1
			cParc := RIGHT(aFaturas[1][nDupl], TamSx3("E1_PARCELA")[1])
		Else
			cParc := SUBSTR(aFaturas[1][nDupl], TamSx3("E1_PREFIXO")[1]+TamSx3("E1_NUM")[1]+1, TamSx3("E1_PARCELA")[1])
			cParc := PADR(cParc, TamSx3("E1_PARCELA")[1])
		Endif
		AADD(aItens, {xFilial("SA6"), cBanco, cAgencia, cConta, xFilial("SE1"), cPref, cDoc, cParc, MVNOTAFIS, xFilial("SA1")})
	Next
//Posiciona no SE1 e SEE 
	DbSelectArea("SE1")
	DbSetOrder(1) // E1_FILIAL+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO
	DbSelectArea("SEE")
	DbSetOrder(1)
	For nPosBol := 1 to Len(aItens)
		If SE1->(DbSeek( aItens[nPosBol, 5] + aItens[nPosBol, 6] + aItens[nPosBol, 7] + aItens[nPosBol, 8] + aItens[nPosBol, 9] ))
			If SEE->(DbSeek(xFilial("SEE") + aItens[nPosBol, 2] +  PADR(aItens[nPosBol, 3], TamSx3("EE_AGENCIA")[1]) + ;
				PADR(aItens[nPosBol, 4], TamSx3("EE_CONTA")[1]) + cSuConta))
				cFuncBol := U_RFuncBol(aItens[nPosBol, 2])
				cFuncBol += "(@oDanfe, SEE->(Recno()))"
				&(cFuncBol)
			EndIf
		EndIf
	Next
