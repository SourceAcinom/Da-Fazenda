...
@collaboration Victor Nigro, Kleber Santos
@since 11/12/2017
@version 1.0
/*/
...
User Function QSQL(cFraseAuto,_aPosicoes,_oJanela)
	...
	If Type("cFilAnt") == "U"
		...
		cModulo 	:= 'SIGAESP' //Nome do M�dulo que ir� fazer a abertura do Smartclient
		...
		oApp:cStartProg    	:= 'u_CFGQSQL' //Instancia a fun��o que ser� executada ap�s a abertura do programa
		oApp:lMessageBar	:= .T.
...
User Function CFGQSQL(cFraseAuto, _aPosicoes, _oJanela)
	...
	Private cCacheHist 	 := ""
	Private	aoBotoes     := {}
	Private	nLargBotao   := 39
	Private	nAltBotao    := 18
	Private	nSpan        := 1

	If Type("oQSQLJanela") != "U"
	...
	If !(FWIsAdmin( __cUserID ) )
		FWAlertError( "Rotina permitida apenas ao Administrador", "CFGQSQL" )
		Return Nil
	EndIf
	...
	If !ExistDir( "\qsql" )
		...
		If !ExistDir( "\qsql" )
			FwAlertError("Erro ao criar diretorio qsql","Erro")
		EndIf

	EndIf

	cLogHist   := "\qsql\cfgqsql_"+RetCodUsr()+fOnlyLetters(GetComputerName())+".log"
	cCacheHist := "\qsql\cfgqsql_"+RetCodUsr()+fOnlyLetters(GetComputerName())+".tmp"
	...
	If Empty(_aPosicoes)
		...
		DEFINE MSDIALOG oQSQLJanela TITLE "DM Tools - QSQL ( v. 06/05/2025a ) "+SPACE(10)+cRPO FROM aSize[1],aSize[2] to aSize[3],aSize[4] PIXEL
        ...
        oEditorSQL := TSimpleEditor():New( SQL_LINE,000,oDlg1,SQL_WIDTH,SQL_HEIGHT )
		oEditorSQL:TextFormat( 2 ) // 1=Html, 2=Texto plano
		oEditorSQL:lAutoIndent := .T.
		oEditorSQL:SetCss("QWidget { "+  SQLCSS() + "}")
		
		If !Empty(cFraseAuto)
			oEditorSQL:Load(cFraseAuto)
		EndIf 
		...
		If "MSSQL" $ AllTrim(Upper(TcGetDb())) .Or. AllTrim(Upper(TcGetDb())) == 'SYBASE' 
			cDicas += "SELECT TOP 10 * FROM"+ CRLF +"WHERE D_E_L_E_T_ = ' '"+ CRLF
		ElseIf "POSTGRE" $ AllTrim(Upper(TcGetDb()))
			cDicas := "SELECT * FROM"+ CRLF +"WHERE D_E_L_E_T_ = ' ' LIMIT 10 "+ CRLF + CRLF 
		Else 
			cDicas += "SELECT * FROM"+ CRLF +"WHERE D_E_L_E_T_ = ' ' FETCH FIRST 10 ROWS ONLY "+ CRLF
		EndIf 

		oResult:AppendText("Ctrl + Down para mostrar os campos (sair ESC)"+ CRLF +"F5 para executar a query"+ CRLF + CRLF + cDicas)

		oEditorSQL:lReadOnly := !lAdmin

		@ 001,001 SAY oSayT     VAR cSayT     OF oDlg2 pixel color CLR_HBLUE

		aButtons := {}

		If Empty(cFraseAuto) .and. File(cCacheHist)

			nHandle := FT_FUse(AllTrim(cCacheHist))

			If nHandle == -1

				MessageBox('Nao foi possivel ler o arquivo ' + cCacheHist , 'AVISO', 16)
			...
			Else
				...
				cTxtLoad :=''
				While ! FT_FEof()
					cTxtLoad += Alltrim(FT_FReadLn())+CRLF
					FT_FSkip()
				EndDo

				oEditorSQL:Load(StrTran(cTxtLoad,CHR(63),CRLF))
		...
		Aadd( aButtons, {"HISTORIC", {||  fRun()}, "SQL (F5)"      , "Executar SQL" , {|| .T.}} )
		...
		For j:= 0 to Min( Len( aButtons ), 15) - 1
			...
			If "File" == aButtons[j+1,3]
				...
				oMenu0304 := tMenuItem():new(oMenu03, "Import CSV", , , , {|| fImpCSV()}, , , , , , , , , .T.)
				oMenu03:add(oMenu0301)
				oMenu03:add(oMenu0302)
				oMenu03:add(oMenu0303)
				oMenu03:add(oMenu0304)
...
Static Function fOpenTab(_cAlias)
	...
	oBrow:lLineDrag  := .T.
	oBrow:lJustIfic  := .T.
	oBrow:blDblClick := {|| fEditCel(oBrow,oBrow:nColPos,.f.)}
...
Static Function fRun(_aPosicoes, _oJanela)
	...
	_cQueryTxt := oEditorSQL:RetTextSel()

	If Len(AllTrim(_cQueryTxt))==0 
		_cQueryTxt := oEditorSQL:RetText()
	EndIf

	_cQueryTxt := StrTran(_cQueryTxt,CHR(160),' ')	
	cSayT :=  " "
	cSayTI := " "
	cSayTF := " "
	...
	If Empty(_aPosicoes)
		...
		MemoWrite(cCacheHist, _cQueryTxt)

	EndIf

	_i:=1
	_cOper  := {"DROP","TRUNCATE","INSERT","UPDATE","DELETE","CREATE","ALTER","GRANT"}
	lSelect := .T.
	...
	If lSelect
		...
		Else
			...
			If Type("oDlg2") <> "O"
				...
				DEFINE MSDIALOG oDlg2 TITLE "Dados" From 001,001 To 020,150
	...
	Else
		cResult := ""
		a_QryTxt := STRTOKARR(STRTRAN(_cQueryTxt,CHR(63),''),";")
		If Len(a_QryTxt) > 0
			cResult := ""
			For _i := 1 to Len(a_QryTxt)
				If Empty(AllTrim(a_QryTxt[_i]))
					loop 
				EndIf
...
Static Function LstCmp()
	...
	If Len(a_Cmps) <> 0
		...
		oEditorSQL:nPos := Len(SubStr(_cQueryTxt,1,n_Pos) + a_Cmps[nList])
...
Static Function Terminal()
	...
	cExpr := StrTran(oEditorSQL:RetTextSel(),chr(160),' ')

	If Len(AllTrim(cExpr))==0 
		cExpr := StrTran(oEditorSQL:RetText(),chr(160),' ')
	EndIf

	cExpr := StrTran(cExpr,chr(10),",")
	
	If Len(AllTrim(cExpr))==0 
		FWAlertError("Selecione o c�digo ADVPL a executar","Aten��o")
		Return
	EndIf
...
Static Function GrvHst(c_Qry)
	...
	If File(cLogHist)
		nHandle := FT_FUse(AllTrim(cLogHist))

		If nHandle == -1
			MessageBox('Nao foi possivel ler o arquivo ' + cLogHist , 'AVISO', 16)
		Else
...
Static function ShwHst(nPar)
	...
	If nPar == 1 .AND. MsgYesNo("Limpa Historico?","Confirma")
		...
		nPos := Max( Rat("\",cLogHist), Rat("/",cLogHist) )

		nStatus := FRename(cLogHist, Substr(cLogHist,1,nPos) + "bkp_" + DTOS(Date()) + "_" + StrTran(Time(),":","_") + "_" + Substr(cLogHist, nPos+1, Len(cLogHist))  )

		If nStatus == 0

			FWAlertInfo( "Conclu�do com sucesso! ", "Aten��o")

		Else

			FWAlertError( "Erro ao excluir historico excluido! " + cLogHist, "Aten��o")
...
Static Function fGetPicture(cNome,xConteudo)
	...
	If Empty(Alltrim(cReturn))
		...
		ElseIf ValType(xConteudo)=="N"
			cReturn := "@E 99,999."+Replicate("9", CASAS_DECIMAIS)
		Else
			cReturn := "@!"
		EndIf
	EndIf
		...
		If lQuery
			...
			For k:=1 To FCount()
				If ValType(FieldGet(k))=="C"
					AADD(aLinha,Alltrim(NoAcento(FieldGet(k))))
				Else
		...
		Else
			...
			For k:= 1 to Len(aSelCampos)
				...
				If ValType(FieldGet(aSelCampos[k,2]))=="C"
					AADD(aLinha,AllTrim(NoAcento(FieldGet(aSelCampos[k,2]))))
				Else
...
Return lReturn .OR. !GetNewPar("MV_X_BLTAB",.F.) // Bloqueia tabelas?
...
Static Function SelTab(aLista)
	...
	AAdd(aStruct, {"Tabela"    , "C", Len( aLista[ 1, 1 ] ), 0})
	AAdd(aStruct, {"Descricao" , "C", 40, 0})
	...
	oTable:AddIndex('01', {'Tabela'} )
	oTable:Create()

	MsAguarde({|| fDadosTab(aLista) },"Selecionando tabelas...","Aguarde...")
...
Static Function fDadosTab( aLista )
	Local i := 0
	...
	If !Eof() .AND. aRet[1]
		...
		Do While !Eof()
				...
				For j:= 1 to FCount()
					...
					Else
						...
						If Empty(cLabel) .OR. cLabel == 'null'
							cLabel := FWhttpEncode(FieldName(j))
						EndIf
			...
			If nRec <= PageSize
				...
				For j:= 1 to FCount()
					oJsonObj[FieldName(j)]   := FWhttpEncode(cValToChar(FieldGet(j)))
				Next
...
Static Function fOnlyLetters( cText )
	Local cRet := ""
	Local j    := 0

	For j:= 1 to Len(cText)

		If Substr(cText,j,1) $ "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
			cRet += Substr(cText,j,1)
		EndIf
...
Static Function fGetCss()
	Local cEstilo1 := ""

	cEstilo1 := ""
	cEstilo1 += "QPushButton{ background-color:  #FFFFFF; "
	...
	cEstilo1 += "background-color: #00A3C6;"
...
Static Function fImpCSV()
	...
	If ParamBox(aPergs ,"Confirmar",aRet)

		oFile := FWFileReader():New( aRet[1] )

		If (oFile:Open())

			oResult:AppendText("[INFO] arquivo aberto "+ aRet[1] + CRLF)
			...
			MsAguarde({|| DbEval({|| AADD(aCombo,{X2_CHAVE,X2_NOME})}) },"Selecionando tabelas...","Aguarde...")

			cArqDic := SelTab(aCombo)

			DbSelectArea( cArqDic )
			DbSetOrder(1) // assume indice 1 como padrao
			DbGoTop()

			oResult:AppendText("[INFO] alias de destino "+ cArqDic + CRLF)
			...
			while (oFile:hasLine())
				...
				if ( nLinha == 1 )
					...
					cLine    := oFile:GetLine()
					aCampos  := MyStrToKArr( StrTran(cLine,"*",""),";")

					Aeval( MyStrToKArr( cLine,";"), {| cCampo | iif("*" $ cCampo, AADD(aIndex,StrTran(cCampo,"*","")), AADD(aIndex,"")) } )

					Aeval( aCampos, { |cCampo| cLog += iif( FieldPos( cCampo ) <= 0, cCampo + " n�o encontrado na tabela " + cArqDic + CRLF,"")})

					If Len(cLog) > 0
						oResult:AppendText("[ERR] " + cLog + CRLF)
						lResult := .F.
						Exit
					EndIf
					...
					For j := 1 to Len(aIndex)
						...
						If !Empty(aIndex[j])
							...
							Do Case
							Case ValType(FieldGet(FieldPos(aIndex[j]))) == "C"
								cChave += Pad(aDados[j], TamSx3(aIndex[j])[1])
							Case  ValType(FieldGet(FieldPos(aIndex[j]))) == "N"
								cChave += cValToChar(aDados[j])
							Case  ValType(FieldGet(FieldPos(aIndex[j]))) == "D"
								cChave += cValToChar(aDados[j])
							EndCase
					...
					Reclock(cArqDic, lNovo)

					For j := 1 to Len(aCampos)

						If FieldPos(aCampos[j]) > 0 .AND. j <= Len(aDados)						
							uConteudo := aDados[j]
							If ValType(FieldGet(FieldPos(aCampos[j]))) == "C"
								FieldPut( FieldPos(aCampos[j]), aDados[j] )
							ElseIf ValType(FieldGet(FieldPos(aCampos[j]))) == "N" .AND. VALTYPE(Val(StrTran(uConteudo,",","."))) == "N"
								FieldPut( FieldPos(aCampos[j]), Val(aDados[j]) )
							ElseIf ValType(FieldGet(FieldPos(aCampos[j]))) == "D" .AND. ("/" $ uConteudo) .AND. VALTYPE(CTOD(uConteudo)) == "D"
								FieldPut( FieldPos(aCampos[j]), CTOD(aDados[j]) )
							ElseIf ValType(FieldGet(FieldPos(aCampos[j]))) == "D" .AND. VALTYPE(STOD(uConteudo)) == "D"
								FieldPut( FieldPos(aCampos[j]), STOD(aDados[j]) )
							EndIf
			...
			oResult:AppendText("[INFO] Terminado com sucesso. " + CRLF)

			MsgInfo("Salvo com Sucesso.")

		Else

			oResult:AppendText("[ERR] Falha ao abrir " + aRet[1])
...
Static Function MyStrToKArr( cText, cSep )
	Local aRet := {}
	Local j    := 0
	Local cAdd := ""
	...
	For j := 1 to Len(cText)
		...
		If Substr(cText,j,1) == cSep
			AADD(aRet, cAdd)
			cAdd := ""
		Else
			cAdd += Substr(cText,j,1)
		EndIf
...
User Function InitTrace( cArquivo )
	Local cMessage := ''
	Local lOk := .F.

	cMessage := iif( cArquivo == nil , 'replay.log', cArquivo)
	TCSqlReplay(2, @cMessage) // define o arquivo
	
    cMessage := '1' // liga o log 
	TCSqlReplay(6, @cMessage)

	cMessage := '1' // quantidade de calls 
	TCSqlReplay(7, @cMessage) 

    cMessage := ''
	lOk := TCSqlReplay(4, @cMessage)  // inicia

Return lOk

User Function FinishTrace()
	Local cMessage := ''

	TCSqlReplay(3, @cMessage) // finaliza

Return MemoRead('replay.log')

Static Function SQLCSS()

	BeginContent var cReturn
			...
			font-weight: bold;
			color:rgb(0, 0, 0);
			background-color:rgb(255, 255, 255); /* Slightly lighter background for code blocks */
			padding: 10px;
			border-radius: 5px;
			overflow-x: auto;
